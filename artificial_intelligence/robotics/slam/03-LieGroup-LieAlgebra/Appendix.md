...
**左雅可比的逆 `J_l(φ)⁻¹` 的闭式形式为**：
$$
J_l(\phi)^{-1} = I - \frac{1}{2}\phi^{\wedge} + \left(\frac{1}{\theta^2} - \frac{1+\cos\theta}{2\theta\sin\theta}\right)(\phi^{\wedge})^2, \quad \theta = ||\phi||
$$
...
**关键性质**：对于单位向量 `a`，其反对称矩阵 `a^` 满足：
*   `(a^)^2 = aaᵀ - I`
*   `(a^)^3 = -a^`
*   这个性质使得 `(a^)^n` 具有周期性，高次项可以被 `a^` 和 `(a^)^2` 线性表示。

**推导过程**：
1.  **代入 `φ^ = θa^`**:
    $$
    \exp(\phi^\wedge) = \boldsymbol{I} + \theta a^\wedge + \frac{1}{2!}\theta^2 (a^\wedge)^2 + \frac{1}{3!}\theta^3 (a^\wedge)^3 + \frac{1}{4!}\theta^4 (a^\wedge)^4 + \dots
    $$

2.  **按 `a^` 的幂次分组**：我们将级数中的项按照 `I`, `a^`, `(a^)^2` 进行重新组合。
    *   **含有 `a^` 的项 (奇数次幂)**：
        `θa^ + (1/3!)θ³(a^)^3 + (1/5!)θ⁵(a^)^5 + ...`
        利用 `(a^)^3 = -a^`, `(a^)^5 = a^` 等，上式变为：
        `θa^ - (1/3!)θ³a^ + (1/5!)θ⁵a^ - ...`
        `= (θ - θ³/3! + θ⁵/5! - ...) a^`
    ...
    *   **含有 `(a^)^2` 的项 (偶数次幂, n≥2)**：
        `(1/2!)θ²(a^)^2 + (1/4!)θ⁴(a^)^4 + (1/6!)θ⁶(a^)^6 + ...`
        利用 `(a^)^4 = -(a^)^2`, `(a^)^6 = (a^)^2` 等，上式变为：
        `(1/2!)θ²(a^)^2 - (1/4!)θ⁴(a^)^2 + (1/6!)θ⁶(a^)^2 - ...`
        `= (θ²/2! - θ⁴/4! + θ⁶/6! - ...) (a^\wedge)^2`
        我们认出括号里的级数是 `1 - cos(θ)` 的泰勒展开。所以这部分是：
        `= (1 - \cos\theta) (a^\wedge)^2`

    *   **常数项 `I`**：级数的第一项 `(1/0!)(φ^)^0 = I`。

3.  **组合结果**：将三部分加起来，我们就得到了**罗德里格斯公式 (Rodrigues' Formula)**：
    $$
    \exp(\phi^\wedge) = \boldsymbol{I} + (\sin\theta) a^\wedge + (1 - \cos\theta) (a^\wedge)^2
    $$
    因为 `φ^ = θa^`，所以 `a^ = (1/θ)φ^`。代入上式，得到以 `φ` 表示的形式：
    $$
    \exp(\phi^\wedge) = \boldsymbol{I} + \frac{\sin\theta}{\theta} \phi^\wedge + \frac{1 - \cos\theta}{\theta^2} (\phi^\wedge)^2
    $$
...
令 `R(φ) = exp(φ^)`。
$$
R(\phi) = \boldsymbol{I} + \frac{\sin\theta}{\theta} \phi^\wedge + \frac{1 - \cos\theta}{\theta^2} (\phi^\wedge)^2
$$
这是一个从 `R³`到`R³ˣ³`的映射。其雅可比是一个三阶张量。为了得到一个 `3x3` 的矩阵，我们通常是计算 `R(φ)` 对 `φ` 的某个分量 `φ_i` 的偏导。
$$
\frac{\partial R}{\partial \phi_i} = \frac{\partial}{\partial \phi_i} \left( \boldsymbol{I} + \frac{\sin\theta}{\theta} \phi^\wedge + \frac{1 - \cos\theta}{\theta^2} (\phi^\wedge)^2 \right)
$$
...
4.  **化简**：利用 `so(3)` 的性质，将这个级数求和，得到闭式解：
    $$
    J_l(\phi) = \boldsymbol{I} + \frac{1-\cos\theta}{\theta^2} \phi^\wedge + \frac{\theta-\sin\theta}{\theta^3} (\phi^\wedge)^2
    $$

**结论**：
*   **罗德里格斯公式**的推导依赖于指数映射的**级数定义**和 `so(3)` 的代数性质 `(a^)^3 = -a^`。
...
5.  **整理成雅可比形式**: 我们的目标是找到一个矩阵 `J_final` 使得 `d(Rp) = J_final * δϕ`。
    我们需要把上式中的 `δϕ` 挪到最右边。这里需要用到一个关键的向量叉乘性质：`a^b = a × b = -b × a = -(b^)a`。
    令 `a = J_l(ϕ)δϕ` 和 `b = Rp`。
    `d(R p) ≈ - (R p)^ (J_l(ϕ)δϕ)`
