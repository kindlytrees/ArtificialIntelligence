...
ICA是解决的一个典型应用场景是多个说话源和多个麦克风混合数据的语音源隔离问题。

ICA的应用中，多个语音源头假设时时间上对齐的，并且对d个混合后的每一个单位数据进行拼接后形成x，这样拼接的顺序可能存在着任意性，去混合矩阵前面加上排列矩阵后的分离效果是一样的，能分离出来，但不能分清源和目标的一一的对应关系（可能存在着多种可能的一一对应关系？）
