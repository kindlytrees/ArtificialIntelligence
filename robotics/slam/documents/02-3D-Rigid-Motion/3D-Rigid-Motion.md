...
- 叉积的代数计算和几何理解
- 齐次坐标的作用和使用场合
- 旋转矩阵，旋转向量，欧拉角，四元数表示三维旋转变换的等价性于相互转换关系
...
1.  **叉乘的矩阵形式 (Skew-Symmetric Matrix)**：
    向量$v$ 与任何向量$p$ 的叉乘$v × p$，可以等效地写成一个反对称矩阵$v^{\wedge}$ (读作 v-hat 或 v-skew) 乘以向量$p$：
   $v × p = v^ * p$
    其中$v^{\wedge}$ 是：
   $$
       [ 0  -vz  vy]
...
$R = v*v^T + s²I + 2s*v^{\wedge} + (v*v^{\wedge}T - ||v||²I)$
$R = 2(v*v^{\wedge}T) + (s² - ||v||²)I + 2s*v{\wedge}$
...
3.  **向量化简**：整理展开后的向量表达式。
4.  **矩阵化**：将点积、叉乘和向量三重积等向量运算，用它们等效的矩阵形式（外积$v*v^{\wedge}T$ 和反对称矩阵$v^{\wedge}$）来代替。
